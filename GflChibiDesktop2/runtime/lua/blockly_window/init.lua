...
-- 通过 transparent_mode.txt（实例目录或 app 根目录，内容为 "ulw"）或环境变量 GFL_TRANSPARENT_MODE 启用。
local ulwMode = (function()
    ...
    -- 工作目录为 app/instances/{id}，app 根需要 ../../transparent_mode.txt
    local m = readMode("transparent_mode.txt") or readMode("../../transparent_mode.txt") or readMode("../transparent_mode.txt")
    if m ~= nil then return m end
    local env = os.getenv("GFL_TRANSPARENT_MODE")
    if env then return env:lower() end
    return nil
end)()
