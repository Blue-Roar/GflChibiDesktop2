...
namespace GflChibiDesktop
{
    static class Ipc
    {
        ...
        extern public static IntPtr hiMQ_openIPC(string name);
        [DllImport("huMessageQueue.dll")]
    ...
    public class ManagedIpc : IDisposable
    {
        ...
        public class IpcWriter : IDisposable
        {
            ...
            /// <summary>结束当前消息。bell=true 时唤醒对端（面板最后一项必须带唤醒信号）。</summary>
            public void End(bool bell)
            {
                if (m.IsDisposed) return;
                Ipc.hiMQ_end(m.ptr, 0, bell ? 1u : 0u);
            }
        }
        ...
        /// <summary>按名字打开已存在的 IPC（由主程序创建并传入名字）。</summary>
        public static ManagedIpc OpenByName(string name)
        {
            IntPtr ptr = Ipc.hiMQ_openIPC(name);
            if (ptr == IntPtr.Zero)
            {
                throw new Exception("无法打开 IPC：" + name);
            }
            return new ManagedIpc(ptr);
        }
