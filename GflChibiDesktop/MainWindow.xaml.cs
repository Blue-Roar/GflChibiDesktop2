...
namespace GflChibiDesktop.Windows
{
    public partial class MainWindow : Window
    {
        ...
        {
            KillEmptyDirectory($@"{AppDomain.CurrentDomain.BaseDirectory}assets/spine");
            ...
            if (item != null)
            {
                ...
                if (!item.ComponentName.Contains("class"))
                {
                    ...
                    if (tagString[8] != null)
                    {
                        ...
                        string cgURL = $@"{AppDomain.CurrentDomain.BaseDirectory}assets/pic/{cg_filename}";
                        try
                        {
                    ...
                    if ((tagString[6] != null) && (tagString[7] != null) && (tagString[11]!=null)) //存在数据
                    {
                        ...
                        if (Directory.Exists($@"{AppDomain.CurrentDomain.BaseDirectory}assets/spine/{tagString[6]}"))
                        {
                            bool checkResult = true;
                            foreach (string filename in tagString[11].Split('|'))
                            {
                                if (!File.Exists($@"{AppDomain.CurrentDomain.BaseDirectory}assets/spine/{tagString[6]}/{filename}"))
                                {
                                    checkResult = false;
                                }
                            }
        ...
        private void btn_loadCG_Click(object sender, RoutedEventArgs e)
        {
            ...
            { 
                cg = true; 
                local_cg = File.Exists($@"{AppDomain.CurrentDomain.BaseDirectory}assets/pic/{tagString[8]}");
            }
            if (tagString[9] != null)
            {
                cg_d = true;
                local_cg_d = File.Exists($@"{AppDomain.CurrentDomain.BaseDirectory}assets/pic/{tagString[9]}");
            }
            ...
            if (cg) //存在立绘
            {
                if (cg && cg_d) //同时存在两种立绘
                {
                    ...
                    {
                        Process.Start($@"{AppDomain.CurrentDomain.BaseDirectory}GflChibiDesktop.CG.exe", $"\"{tagString[5]}\" \"{AppDomain.CurrentDomain.BaseDirectory}assets/pic/\" \"{tagString[8]}\" \"{tagString[9]}\"");
                        //new WindowCG().LoadCG(tagString[5], $@"{AppDomain.CurrentDomain.BaseDirectory}assets/pic/", tagString[8], tagString[9]);
                    }
                    else
                    {
                        ...
                        {
                            Process.Start($@"{AppDomain.CurrentDomain.BaseDirectory}GflChibiDesktop.CG.exe", $"\"{tagString[5]}\" \"{Properties.Settings.Default.DownloadSource}/pic/\" \"{tagString[8]}\" \"{tagString[9]}\"");
                ...
                else //只有一种立绘
                {
                    ...
                    {
                        Process.Start($@"{AppDomain.CurrentDomain.BaseDirectory}GflChibiDesktop.CG.exe", $"\"{tagString[5]}\" \"{AppDomain.CurrentDomain.BaseDirectory}assets/pic/\" \"{tagString[8]}\"");
                        //new WindowCG().LoadCG(tagString[5], $@"{AppDomain.CurrentDomain.BaseDirectory}assets/pic/", tagString[8]);
                    }
                    else
                    {
                        if ((bool)chb_save_cg.IsChecked)
                        ...
                        {
                            Process.Start($@"{AppDomain.CurrentDomain.BaseDirectory}GflChibiDesktop.CG.exe", $"\"{tagString[5]}\" \"{Properties.Settings.Default.DownloadSource}/pic/\" \"{tagString[8]}\"");
        ...
        private void DownloadCG(string[] tagString)
        {
            ...
            if (!Directory.Exists($@"{AppDomain.CurrentDomain.BaseDirectory}assets/pic"))
            {
                Directory.CreateDirectory($@"{AppDomain.CurrentDomain.BaseDirectory}assets/pic");
            }
            sp_downloader.Visibility = Visibility.Visible;
            pb_loader.IsIndeterminate = false;
            pb_loader.Value = 0;
            ...
            if (tagString[9] != null)
            {
                ...
                pb_loader.Maximum = 2;
                HttpClass.DownloadFile($"{cg_url}/{tagString[9]}", $@"{AppDomain.CurrentDomain.BaseDirectory}assets/pic/{tagString[9]}", pb_downloader, lbl_downloader);
                pb_loader.Value++;
            }
            lbl_loader.Content = $"正在下载 {tagString[5]} 的立绘数据";
            HttpClass.DownloadFile($"{cg_url}/{tagString[8]}", $@"{AppDomain.CurrentDomain.BaseDirectory}assets/pic/{tagString[8]}", pb_downloader, lbl_downloader);
            pb_loader.Value++;
            sp_downloader.Visibility = Visibility.Collapsed;
        ...
        private void LoadInternalSpine(string[] tagString, bool dormMode)
        {
            ...
            if (dormMode)
            {
                if (File.Exists($@"{AppDomain.CurrentDomain.BaseDirectory}assets/spine/{tagString[6]}/{tagString[10]}.atlas"))
                { AtlasFile = $@"assets/spine/{tagString[6]}/{tagString[10]}.atlas"; }
                if (File.Exists($@"{AppDomain.CurrentDomain.BaseDirectory}assets/spine/{tagString[6]}/{tagString[10]}.skel"))
            ...
            //}
            //if (File.Exists($@"{AppDomain.CurrentDomain.BaseDirectory}assets/name.txt")) { File.Delete($@"{AppDomain.CurrentDomain.BaseDirectory}assets/name.txt"); }
            File.WriteAllText($@"{AppDomain.CurrentDomain.BaseDirectory}assets/name.txt", DisplayName);
            //if (File.Exists($@"{AppDomain.CurrentDomain.BaseDirectory}assets/model.conf.json")) { File.Delete($@"{AppDomain.CurrentDomain.BaseDirectory}assets/model.conf.json"); }
            File.WriteAllText($@"{AppDomain.CurrentDomain.BaseDirectory}assets/model.conf.json", "{\"skeleton\":\"" + SpineFile + "\",\"type\":\"skel\",\"atlas\":\"" + AtlasFile + "\",\"h\":448,\"w\":448,\"x\":224,\"y\":224}");
            ...
            if (chb_force_load.IsChecked.Value)
            {
                ...
                ProcessStartInfo startInfo = new ProcessStartInfo($@"{AppDomain.CurrentDomain.BaseDirectory}luajit.exe", "main.lua");
            ...
            if (tagString[11].Split('|').Count() > 0)
            {
                if (CheckIsUrlFormat(downloadSource))
                {
                    ...
                    foreach (string filename in tagString[11].Split('|'))
                    {
                        ...
                        if (!Directory.Exists($@"{AppDomain.CurrentDomain.BaseDirectory}assets/spine/{tagString[6]}"))
                        {
                            Directory.CreateDirectory($@"{AppDomain.CurrentDomain.BaseDirectory}assets/spine/{tagString[6]}");
                        }

                        HttpClass.DownloadFile($"{downloadSource}/{filename}", $@"{AppDomain.CurrentDomain.BaseDirectory}assets/spine/{tagString[6]}/{filename}", pb_downloader, lbl_downloader);
        ...
        {
            if (Directory.Exists($@"{AppDomain.CurrentDomain.BaseDirectory}assets/spine/{dummy}"))
            {
                Directory.Delete($@"{AppDomain.CurrentDomain.BaseDirectory}assets/spine/{dummy}", true);
            }
            tvAfterSelect();
        }
